...
def decode_avro(raw_schema):
    encoded = raw_schema.encode("utf-8")
    byt = io.BytesIO(encoded)
    result = schemaless_reader(byt, schema)
    return result
...
metric_ds_with_windowing = parsed_ds.key_by(lambda x: x['metric']).window(TumblingEventTimeWindows.of(Time.seconds(10))).apply(CollectAll())
